...
packages/
  proxy/
    ...
    test/
      ...
      integration.test.ts
deploy/
  colosseum-proxy.service                 ← systemd unit file
...
**Files:**
- Create: `deploy/colosseum-proxy.service`
...
```ini
[Unit]
Description=Colosseum LLM Proxy
After=network.target

[Service]
Type=simple
User=ubuntu
WorkingDirectory=/home/ubuntu/colosseum
ExecStart=/home/ubuntu/.bun/bin/bun run packages/proxy/src/index.ts
Restart=always
RestartSec=5
EnvironmentFile=/home/ubuntu/colosseum/packages/proxy/.env
...
```
Save to `deploy/colosseum-proxy.service`.
...
```bash
git remote add origin https://github.com/YOUR_ORG/colosseum.git
...
```bash
ssh ubuntu@<VPS_IP>
curl -fsSL https://raw.githubusercontent.com/YOUR_ORG/colosseum/main/deploy/deploy.sh | bash
```

If the script exits asking you to set the `.env`, do so:
```bash
nano ~/colosseum/packages/proxy/.env
# Set PORT=3000, DB_PATH=./colosseum.db, BASE_RPC_URL=https://mainnet.base.org
...
```bash
cd /Users/steve/code/colosseum && bun test
```
