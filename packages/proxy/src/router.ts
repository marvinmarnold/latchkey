...
export function selectListing(db: Database, modelId: string): Listing | null {
  ...
  // Weighted score: (price_input + price_output) per-million / reliability — cheaper AND reliable wins.
  return (
    db
      .query<Listing, [string]>(
        `SELECT l.* FROM listings l
         ...
         ORDER BY ((l.price_input_usd_per_million + l.price_output_usd_per_million) / MAX(l.reliability, 0.01)) ASC
         LIMIT 1`,
      )
      .get(modelId) ??
    db
