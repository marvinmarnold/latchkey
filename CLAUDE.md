...
**Exception — infrastructure-only vars** (e.g. `CLOUDFLARE_API_TOKEN`, `DEPLOY_PASSWORD`): skip steps 1 and 2. These are consumed by deploy tooling, not the proxy process itself. Only add to `.env.example` and handle in `deploy/deploy.sh`.
